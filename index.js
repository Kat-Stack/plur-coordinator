...
async function executePassedProposal(proposal, channel, client) {
    if (proposal.proposal_type === 'global_broadcast') {
        ...
        // Fetch the sender's Short ID to append to the broadcast
        let senderNode = await client.db.get(`SELECT short_id FROM node_registry WHERE channel_id = ?`, [proposal.channel_id]);
        
        // 🛡️ AUTO-HEAL: If this channel is missing from the registry, force register it right now.
        if (!senderNode) {
            try {
                await client.db.run(`INSERT INTO node_registry (channel_id, plur_name) VALUES (?, ?)`, [proposal.channel_id, channel.name]);
                senderNode = await client.db.get(`SELECT short_id FROM node_registry WHERE channel_id = ?`, [proposal.channel_id]);
                console.log(`[AUTO-HEAL] Registered ghost node ${channel.name} as ID #${senderNode.short_id}`);
            } catch (e) {
                console.error("[AUTO-HEAL FAILED]", e);
            }
        }

        const bridgeInstruction = senderNode ? `\n\n*🔗 Join Plur, run:* \`/join_plur ${senderNode.short_id}\`` : '';
            ...
            try {
                ...
                if (targetChannel) {
                    const globalEmbed = new EmbedBuilder()
                        ...
                        .setDescription(`${proposal.payload}${bridgeInstruction}`)
                        .setColor('#2ecc71')
                        .setFooter({ text: '📡 Virtnet'})
                        .setTimestamp();
...
client.once('clientReady', async (c) => { 
    try {
        ...
        await client.db.exec(`
            ...
            -- NEW: The central registry mapping Discord Channels to Short IDs
